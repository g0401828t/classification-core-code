...
parser.add_argument("--hp_bs", type=int, default=16, help="setting batch size")
...
data_dir = "../tomato"
init_dataset = datasets.ImageFolder(data_dir)
ratio = 0.8
lengths = [int(len(init_dataset)*ratio), int(len(init_dataset)*(1-ratio)/2), len(init_dataset)-int(len(init_dataset)*(ratio))-int(len(init_dataset)*(1-ratio)/2)]
train, val, test = torch.utils.data.random_split(init_dataset, lengths)
train_data = MyLazyDataset(train, transform['train'])
valid_data = MyLazyDataset(val, transform['val'])
test_data = MyLazyDataset(test, transform['val'])
...
print("%d, %d, %d"%(len(train_data),len(valid_data),len(test_data)))
